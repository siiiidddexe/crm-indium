...
// Get email templates
$emailTemplates = $showEmailBtn ? getEmailTemplates() : [];
                ...
                <div class="calling-card bg-white rounded-2xl border-2 border-gray-200 overflow-hidden transition-all duration-300 <?= $index === 0 ? 'border-black shadow-lg' : 'opacity-60' ?>"
                    ...
                    <div class="card-actions border-t border-gray-100 p-4 bg-gray-50">
                        ...
                        <div class="state-call flex gap-2">
                            ...
                            <button
                                onclick="openTemplatePicker('<?= sanitize($contact['phone']) ?>', '<?= sanitize($contact['name']) ?>')"
                            ...
                            <?php if ($showEmailBtn): ?>
                            <button
                                onclick="openEmailModal(<?= $contact['id'] ?>, '<?= sanitize($contact['email'] ?? '') ?>', '<?= sanitize($contact['name']) ?>')"
                                class="py-4 px-3 bg-blue-500 text-white rounded-xl font-bold text-base flex items-center justify-center gap-1 hover:bg-blue-600 active:scale-[0.98] transition-all" title="Send Email">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <button onclick="openNotesModal(<?= $contact['id'] ?>)"
                                class="py-4 px-3 bg-purple-500 text-white rounded-xl font-bold text-base flex items-center justify-center gap-1 hover:bg-purple-600 active:scale-[0.98] transition-all"
                            ...
                            <button onclick="openMoveModal(<?= $contact['id'] ?>)"
                                class="py-4 px-3 bg-orange-500 text-white rounded-xl font-bold text-base flex items-center justify-center gap-1 hover:bg-orange-600 active:scale-[0.98] transition-all"
                        ...
                        <div class="state-endcall hidden">
                            <div class="flex gap-2">
                                ...
                                <button onclick="openTemplatePicker('<?= sanitize($contact['phone']) ?>', '<?= sanitize($contact['name']) ?>')"
                                    class="py-4 px-3 bg-emerald-600 text-white rounded-xl font-bold text-base flex items-center justify-center gap-1 hover:bg-emerald-700 active:scale-[0.98] transition-all"
                                    title="Send WhatsApp">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                ...
                                <?php if ($showEmailBtn): ?>
                                <button onclick="openEmailModal(<?= $contact['id'] ?>, '<?= sanitize($contact['email'] ?? '') ?>', '<?= sanitize($contact['name']) ?>')"
                                    class="py-4 px-3 bg-blue-500 text-white rounded-xl font-bold text-base flex items-center justify-center gap-1 hover:bg-blue-600 active:scale-[0.98] transition-all" title="Send Email">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
...
<div id="notesModal"
    ...
    <div class="bg-white rounded-t-3xl sm:rounded-2xl w-full sm:max-w-md max-h-[85vh] overflow-hidden flex flex-col animate-slide-up">
        <div class="p-5 border-b border-gray-200 bg-white sticky top-0">
            ...
            <p class="text-sm text-gray-500 mt-1" id="notesModalContact"></p>
        </div>
        <div class="flex-1 overflow-y-auto p-4">
            <!-- Past notes -->
            <div id="notesList" class="space-y-3 mb-4">
                <div class="text-center text-gray-400 text-sm py-4" id="notesLoading">Loading notes...</div>
            </div>
            <!-- Add new note -->
            <div class="border-t border-gray-100 pt-4">
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Add a note</label>
                <textarea id="newNoteText" rows="3"
                    class="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-black focus:border-black resize-none"
                    placeholder="What did you discuss? Any important context..."></textarea>
                <button onclick="saveNote()"
                    class="mt-2 w-full py-2.5 bg-black text-white rounded-xl font-medium text-sm hover:bg-gray-800 transition-colors">
                    Save Note
                </button>
            </div>
        </div>
        <div class="p-4 border-t border-gray-100 bg-gray-50">
            <button onclick="hideModal('notesModal')"
                class="w-full py-3 bg-gray-200 rounded-xl font-medium hover:bg-gray-300 transition-colors">Close</button>
        </div>
    </div>
</div>
<?php endif; ?>
...
<div id="emailModal"
    class="fixed inset-0 bg-black bg-opacity-60 z-50 hidden flex items-end sm:items-center justify-center"
    data-modal-backdrop>
    <div class="bg-white rounded-t-3xl sm:rounded-2xl w-full sm:max-w-md max-h-[90vh] overflow-hidden flex flex-col animate-slide-up">
        <div class="p-5 border-b border-gray-200">
            <h2 class="text-xl font-bold text-black">Send Email</h2>
            <p class="text-sm text-gray-500 mt-1" id="emailModalContact">Select template &amp; recipient</p>
        </div>
        <div class="p-4 space-y-3 overflow-y-auto flex-1">
            <div>
                ...
                <input type="email" id="emailTo" placeholder="recipient@example.com"
                    class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-black" />
            </div>
            <div>
                ...
                <select id="emailTemplateSelect"
                    ...
                    <option value="<?= $tpl['id'] ?>"><?= sanitize($tpl['name']) ?> — <?= sanitize($tpl['subject']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
        <div class="p-4 border-t border-gray-100 bg-gray-50 flex gap-3">
            <button onclick="hideModal('emailModal')" class="flex-1 py-3 bg-gray-200 rounded-xl font-medium hover:bg-gray-300 transition-colors">Cancel</button>
            <button onclick="sendEmailFromModal()" id="emailSendBtn" class="flex-1 py-3 bg-blue-500 text-white rounded-xl font-bold hover:bg-blue-600 transition-colors">Send Email</button>
        </div>
    </div>
</div>
<?php endif; ?>
...
<script>
    ...
    const showEmailBtn = <?= $showEmailBtn ? 'true' : 'false' ?>;
    const emailTemplates = <?= json_encode($emailTemplates) ?>;
    let waPhone = null;
    let waName = null;
    let emailContactId = null;
    let emailContactName = null;

    function openEmailModal(contactId, contactEmail, contactName) {
        emailContactId = contactId;
        emailContactName = contactName;
        document.getElementById('emailModalContact').textContent = 'Sending to: ' + contactName;
        document.getElementById('emailTo').value = contactEmail || '';
    ...
    function sendEmailFromModal() {
        ...
        fetch('<?= APP_URL ?>/api/email.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'send', contact_id: emailContactId, template_id: parseInt(templateId), to })
    ...
    function openNotesModal(contactId) {
        notesContactId = contactId;
        const card = document.querySelector(`[data-id="${contactId}"]`);
        const name = card ? card.dataset.name : 'Contact';
        document.getElementById('notesModalContact').textContent = name;
        document.getElementById('newNoteText').value = '';
        document.getElementById('notesList').innerHTML = '<div class="text-center text-gray-400 text-sm py-4">Loading...</div>';
                ...
                if (data.notes && data.notes.length > 0) {
                    list.innerHTML = data.notes.map(n => `
                        <div class="bg-gray-50 rounded-xl p-3">
                            <p class="text-sm text-black whitespace-pre-wrap">${escapeHtml(n.note)}</p>
                            <p class="text-xs text-gray-400 mt-1.5">${escapeHtml(n.author)} · ${escapeHtml(n.created_at)}</p>
                        </div>`).join('');
                } else {
                    list.innerHTML = '<p class="text-center text-gray-400 text-sm py-4">No notes yet. Add the first one!</p>';
                }
            })
            .catch(() => {
                document.getElementById('notesList').innerHTML = '<p class="text-center text-red-400 text-sm py-4">Could not load notes.</p>';
            });
    }

    function saveNote() {
        const note = document.getElementById('newNoteText').value.trim();
        if (!note) { showToast('Please type a note first.', 'error'); return; }

        fetch('<?= APP_URL ?>/api/notes.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'add', contact_id: notesContactId, note })
        }).then(r => r.json()).then(data => {
            if (data.success) {
                document.getElementById('newNoteText').value = '';
                ...
                div.innerHTML = `<p class="text-sm text-black whitespace-pre-wrap">${escapeHtml(n.note)}</p>
                    <p class="text-xs text-gray-400 mt-1.5">${escapeHtml(n.author)} · ${escapeHtml(n.created_at)}</p>`;
                list.prepend(div);
    ...
    // Load more cards when down to last 5
        ...
        return `<div class="calling-card bg-white rounded-2xl border-2 border-gray-200 overflow-hidden transition-all duration-300 opacity-60"
            ...
            <div class="card-actions border-t border-gray-100 p-4 bg-gray-50">
                <div class="state-call flex gap-2">
                    ...
                    ${showEmailBtn ? `
                    <button onclick="openEmailModal(${contact.id}, '${escapeHtml(contact.email || '')}', '${escapeHtml(contact.name)}')"
                        class="py-4 px-3 bg-blue-500 text-white rounded-xl font-bold text-base flex items-center justify-center gap-1 hover:bg-blue-600 active:scale-[0.98] transition-all" title="Send Email">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    ${notesEnabled ? `
                    <button onclick="openNotesModal(${contact.id})"
                        class="py-4 px-3 bg-purple-500 text-white rounded-xl font-bold text-base flex items-center justify-center gap-1 hover:bg-purple-600 active:scale-[0.98] transition-all" title="Add / view notes">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
