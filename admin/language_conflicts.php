...
$pendingCount = getPendingMoveRequestsCount();
...
<div class="p-4 sm:p-6 lg:p-8">
    ...
    <!-- Bulk Actions (visible when pending) -->
    <?php if ($filterStatus === 'pending' && !empty($requests)): ?>
            ...
            <?php foreach ($requests as $req): ?>
                <div class="bg-white rounded-2xl border border-gray-200 p-5 hover:shadow-md transition-shadow"
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                        <!-- Checkbox for bulk (only pending) -->
                        ...
                        <?php if ($req['status'] === 'pending'): ?>
                            <div class="flex gap-2">
                                <button onclick="autoAssign(<?= $req['id'] ?>)"
                                    ...
                                    title="Randomly assign to a speaker of this language">
                                    ⚡ Auto
                                </button>
...
<script>
    ...
    function autoAssign(requestId) {
        fetch('<?= APP_URL ?>/api/language.php', {
            ...
            if (data.success) {
                showToast('Assigned to: ' + data.assigned_to);
    ...
    function bulkApprove() {
        ...
        if (!assignTo) { showToast('Please select an employee to assign to', 'error'); return; }
        ...
        fetch('<?= APP_URL ?>/api/language.php', {
            ...
            body: JSON.stringify({
                ...
                assign_to: parseInt(assignTo),
                approved_by: adminId
            })
        }).then(r => r.json()).then(data => {
            if (data.success) {
                showToast(data.approved_count + ' requests approved and assigned!');
                setTimeout(() => location.reload(), 1000);
            } else {
                showToast('Error: ' + data.error, 'error');
            }
