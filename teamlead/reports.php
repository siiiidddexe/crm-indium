<?php
$pageTitle = 'Team Reports - Obsiguard CRM';
...
if (isset($_GET['export'])) {
    ...
    // Verify employee is in this TL's team or is the TL
    $employee = db()->fetch("SELECT * FROM users WHERE id = ? AND (teamlead_id = ? OR id = ?)", [$employeeId, $userId, $userId]);
    if (!$employee) {
        die('Unauthorized');
    }
...
// Get team members + self with stats
$employees = db()->fetchAll("
    ...
    FROM users u
    WHERE u.is_active = 1 AND (u.teamlead_id = ? OR u.id = ?)
    ORDER BY CASE WHEN u.id = ? THEN 0 ELSE 1 END, u.name
", [$filterDateFrom, $filterDateTo, $userId, $userId, $userId]);
...
<div class="p-4 sm:p-6 lg:p-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-black">Team Reports</h1>
            <p class="text-gray-500 mt-1">View your team's performance and progress</p>
        </div>
                    ...
                    <div class="flex items-center gap-4 flex-1">
                        ...
                        <div>
                            <div class="flex items-center gap-2">
                                ...
                                <span class="px-2 py-0.5 rounded-full text-xs font-medium <?= $emp['id'] == $userId ? 'bg-blue-100 text-blue-700' : ($emp['role'] === 'teamlead' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-700') ?>">
                                    <?= $emp['id'] == $userId ? 'You' : ucfirst($emp['role']) ?>
                                </span>
                            </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center gap-3 py-2 px-3 bg-gray-50 rounded-lg">
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-gray-400 mt-1 italic"><?= sanitize($call['notes']) ?></p>
                                <?php endif; ?>
                            </div>
