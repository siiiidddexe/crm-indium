...
> **Version:** 1.1 (Flask + Gmail OAuth2 + SMTP/Hostinger)

---

## What is NexoMailer?

NexoMailer is a **multi-tenant email-as-a-service API** that sends transactional emails through either Gmail accounts (via Google OAuth2) or any SMTP provider (e.g. Hostinger, SendGrid, custom domain). It's designed so any app (web, mobile, backend) can send OTP codes, password reset links, and custom HTML emails by making a single HTTP POST request.

**Key architecture:**
- Each **API key** represents a client/project (e.g. "Servleash", "MyStore")
- Each API key has its own **email account pool** — Gmail accounts and/or SMTP accounts, mixed freely
- Emails are sent via the Gmail API **or** SMTP — NexoMailer picks the best available account automatically
...
- **Fully backward compatible** — existing Gmail accounts keep working unchanged
...
NexoMailer enforces per-account rate limits (applies to both Gmail and SMTP accounts equally):

| Account Type | Per Minute | Per Day |
|-------------|-----------|---------|
| `workspace` (Google Workspace or high-volume SMTP) | 30 | 2,000 |
| `standard` (free Gmail or standard SMTP) | 20 | 500 |
...
Multiple accounts (Gmail and/or SMTP) can be linked to a single API key — NexoMailer automatically picks the account with the lowest current load (smart load balancing across providers).
...
3. Click the API key → **"+ Connect Gmail (OAuth)"** → Sign in with Google
4. The Gmail account is now linked and ready to send emails for that API key

### Linking SMTP / Hostinger Accounts (via Dashboard)

1. Log into the dashboard
2. Click any API key → **"+ Add SMTP / Hostinger"**
3. Enter your Hostinger (or any SMTP) credentials
4. Click **Save SMTP Account**

You can mix Gmail and SMTP accounts on the same API key — NexoMailer load-balances across all of them automatically.

---

## API Key Structure (Internal Reference)

Each API key stores this structure in `data/api_keys.json`. Accounts can be Gmail or SMTP — both live in the same `accounts` array and are load-balanced together.
...
- `provider` — `"gmail"` (default, backward compat) or `"smtp"`
- `name` — Display name for the client/project
- `company` — Used as the default `app_name` in email templates
- `account_type` — `"standard"` (500/day) or `"workspace"` (2000/day)

---

## Adding SMTP / Hostinger Accounts

### Via Dashboard (recommended)
1. Open the dashboard → click any API key
2. Click **"+ Add SMTP / Hostinger"**
3. Fill in: email, SMTP host, port, username, password
4. Click **Save SMTP Account**
...
```bash
curl -X POST https://nexomail.logiclaunch.in/api/keys/nxm_YOUR_KEY/accounts/smtp \
  -H "Content-Type: application/json" \
...
**Response:**
```json
{ "success": true, "email": "hello@yourdomain.com", "provider": "smtp" }
```

> **Note:** This endpoint requires admin authentication (dashboard login), not an API key. It's for admin use only — your client apps never call this directly.
...
A: It depends on the provider. Gmail API sends have excellent deliverability. SMTP via Hostinger with a proper domain (SPF, DKIM, DMARC records set) also delivers well. Avoid sending bulk/marketing content through either.
...
**Q: Can I mix Gmail and SMTP accounts on the same API key?**
A: Yes. You can add any combination — NexoMailer picks the best available account automatically, regardless of provider.
